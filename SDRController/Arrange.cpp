...
map< pair<Point_t, int>, vector<Point_t> > routeCache;
pair<Point_t, int> currentCenter;
vector<bool> used;
...
void Arrange_t::build(SwitchesShow *ss)
{
		...
		if(!coilList[i].fresh)
		{
			...
			for (int j = 0; j < pointList.size(); ++j)
			{
				...
				if (j > 0)
				{
					...
					route.insert(
						make_pair(
							Point2Id[pointList[j]], 
							make_pair(Point2Id[pointList[j - 1]], 7)
						)
					);
				}
			}
	...
	for (int i = 0; i < NofNode; ++i)
		if (Point2Node[i] == i && PluginPoint.find(i) == PluginPoint.end() && !used[i])  // neither source nor terminal
		{
...
#ifdef ALGORITHM2
	for (int i = 0; i < NofNode; ++i)
		if (Point2Node[i] == i && !used[i])  // not terminal
		{
			addEdge(i, i + NofNode, 1, 0);
			addEdge(i + NofNode, i, 0, 0);
		}
...
pair<QString, int> Arrange_t::dfs(SwitchesShow *ss, int x, int inDir)
{
	...
	if (it == route.end())
	{
		...
		currentCenter = make_pair(coilList[CoilId].o, coilList[CoilId].pattern);
		routeCache[currentCenter].clear();
	...
	else
	{
		...
		for (int i = 0; i < NofDirection; ++i)
		{
			...
			if (it != PluginPoint.end() && it->second == ret.first)
			{
				...
				routeCache[currentCenter].push_back(Id2Point[q]);
				return make_pair("Finished", 0);
			}
		}
#endif
...
void Arrange_t::flow(SwitchesShow *ss)
{
	...
	*/
	...
	for (int i = head[S]; i != -1; i = e[i].next)
		if(e[i].c == 1)
			e[i].c = -1; // A dirty hack. Actually it needs to be 0, but there will be some problems to be 0. Set to -1 happend works.
#endif
...
void Arrange_t::arrange_algorithm(SwitchesShow* ss)
{
	route.clear();
#ifdef ALGORITHM1
	build(ss);
